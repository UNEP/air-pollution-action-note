<script lang="ts" context="module">
  ...
  <script lang="ts">
    ...
    const ozoneSentences = "Breathing in ground-level ozone can irritate the lungs and cause a range of health problems. Currently, <strong>chronic obstructive pulmonary disease</strong> is the only disease for which the burden of disease attributable to ozone exposure can be quantified. ";
    ...
    <h3 class="note col-text"><strong>Percent of deaths</strong> from each disease <strong>attributable to {isOzone ? "long-term exposure to outdoor ground-level ozone air pollution" : "fine particle outdoor air pollution"}</strong> in {isOzone ? "2020" : "2023"} (age-standardized).</h3>
  
    <div class="flex-container">
      {#each serializedData as d}
        <WaffleChart percentage={d[1]} cause={d[0]} isOzone={isOzone} isSingle={true}/>
      {/each}
    </div>
