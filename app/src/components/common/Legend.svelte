...
<script lang="ts">
  ...
  import Checkbox from '../Checkbok.svelte';
  ...
  const calcLinearLabels = (labels: string[]) => {
    let ret = [];
    let normalizedLabels = labels.map((label) =>
    ...
    ret.push(
      ((linearDomain[1] - normalizedLabels[normalizedLabels.length - 1]) /
        globalDistance) *
        100
    );
...
<div class="legend-container" class:grid={checkbox}>
      ...
      {#each colors as c, i}
        ...
        >
          {#if internalLabels}
            ...
            {/if}
            <div class="note internal-label">
              {internalLabels[i] ? internalLabels[i].label : ""}
            </div>
          {/if}
      ...
      {#each colors as c, i}
        ...
        >
          <div
            ...
            background-size: {c !== 'url(#hash--windblown)'
              ? ''
              : '5.66px 5.66px'};
  ...
  {#if checkbox}
    ...
    <div class="checkbox">
      <Checkbox label="Show total number as outline" bind:checked />
    </div>
  {/if}
</div>
